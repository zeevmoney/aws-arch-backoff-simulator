...
class Backoff:
    def __init__(self, base, cap):
        # base for calculating backoff
        self.base = base
        # Backoff max Capacity - capped exponential backoff means that clients multiply
        # their backoff by a constant after each attempt, up to some maximum value
        self.cap = cap  #
...
def setup_sim(clients, backoff_class, ts_f, stats):
    net = Net(10, 2)
    queue = []
    server = OccServer(net, stats, ts_f)
    for i in xrange(0, clients):
        client = OccClient(server, net, backoff_class(5, 2000))  # backoff_class(base, capacity)
